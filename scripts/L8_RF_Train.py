...
# Training Data from Landsat-8 Data
...
import datetime
import time
import joblibfrom sklearn.model_selection import RandomizedSearchCV
...
results_txt = path_to_restults + 'L8_Training_Algorithm.txt'

# path to predefined training data
path_to_arrays = ''

training_data_name = 'L8_Img_Array.npz' # npz is numpy zip file extension
label_data_name = 'L8_ROI_Array.npz' # npz is numpy zip file extension

rf_classifier_name = 'L8_RF_Optimized_CV'
...
X = shape_img[roi > 0, :] #this is simply the training values.
y = roi[roi > 0] #this is simply the roi values.
...
num_water = y[y == 2]
num_not_water = y[y == 1]
print('There are {sz} samples in the Water dataset'.format(sz=num_water.shape), file=open(results_txt, "a"))
print('There are {sz} samples in the Not-Water dataset'.format(sz=num_not_water.shape), file=open(results_txt, "a"))
...
'''
n_estimators = [90,95,100,125,150,175,200,250,300,350,400,450,500]
